...
export default class AutoDocumentGeneratorPlugin extends Plugin {
	...
	async onload() {
		...
		this.addRibbonIcon('pencil', 'Docchain写作助手', async () => {
			if (isshown == false) {
	...
	showCustomModalInOutline() {
		const modal = new AutoDocumentGeneratorModal(this.app, this.settings.usertoken, this.settings.serveraddress, this.settings.username, this.settings.password, this.settings.topicid);	
		modal.open();;
	}
	async loadSettings() {
...
class AutoDocumentGeneratorModal extends Modal {
	...
	onOpen() {
		...
		contentEl.createEl('h1', { text: 'Docchain写作助手' });
		...
		let documentButton = buttonDiv.createEl('button', { text: '生成文档' });
		documentButton.title = '逐段生成文档，以二级标题为单位';
		documentButton.classList.add('button-style');
		let rgButton = buttonDiv.createEl('button', { text: '重新生成' });
		...
		let resetButton = buttonDiv.createEl('button', { text: '返回目录' });
		resetButton.title = '清除已生成的文档内容，返回已保存的目录，接下来可以修改目录或重新生成文档';
