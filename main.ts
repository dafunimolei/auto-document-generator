...
interface AutoDocumentGeneratorSettings {
	usertoken: string;
	serveraddress:string
}
const DEFAULT_SETTINGS: AutoDocumentGeneratorSettings = {
	usertoken: '',
	serveraddress: ''
};
export default class AutoDocumentGeneratorPlugin extends Plugin {
	settings: AutoDocumentGeneratorSettings;
	...
	async onload() {
		...
		this.addSettingTab(new ADGSettingTab(this.app, this));
	...
	showCustomModalInOutline() {
		const modal = new AutoDocumentGeneratorModal(this.app, this.settings.usertoken, this.settings.serveraddress);	
		modal.open();;
	}
	async loadSettings() {
...
class AutoDocumentGeneratorModal extends Modal {
	...
	onOpen() {
		...
		inputInfoDiv.classList.add('input-info-div');
		...
		this.input_username.classList.add('input');
		...
		this.input_password.classList.add('input');
		...
		this.input_topicid.classList.add('input');
		this.input_topicid.focus();
		const buttonDiv = contentEl.createEl('div');
		buttonDiv.classList.add('button-div');
		let resetButton = buttonDiv.createEl('button', { text: 'Reset' });
		resetButton.title = 'Reset to the index and prepare to generate new document';
		resetButton.classList.add('button-style');
		let indexButton = buttonDiv.createEl('button', { text: 'Index' });
		indexButton.title = 'Generate new index';
		indexButton.classList.add('button-style');
		let documentButton = buttonDiv.createEl('button', { text: 'Document' });
		documentButton.title = 'Generate the next paragraph according to the existing index';
		documentButton.classList.add('button-style');
		let safButton = buttonDiv.createEl('button', { text: 'Save' });
		safButton.title = 'Save this file to another file';;
		safButton.classList.add('button-style');
		let rgButton = buttonDiv.createEl('button', { text: 'Regenerate' });
		rgButton.title = 'Regenerate this paragraph';
		rgButton.classList.add('button-style');
		let chatButton = buttonDiv.createEl('button', { text: 'Chat' });
		chatButton.title = 'Chat with ChatGPT to impovish the document';
		...
		this.chat_area.placeholder = 'GPT ready to answer:';
...
class ADGSettingTab extends PluginSettingTab {
	plugin: AutoDocumentGeneratorPlugin;
	constructor(app: App, plugin: AutoDocumentGeneratorPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	}
	display(): void {
